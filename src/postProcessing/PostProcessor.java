...
public class PostProcessor {
    ...
    private void connectToDatabase() throws ClassNotFoundException, SQLException{
        ...
        connection = DriverManager.getConnection("jdbc:sqlite:complete-64.db");
        connection.setAutoCommit(false);
    }
    
    private void overallButterfly() throws SQLException{
        statement = connection.createStatement();
        statement.executeUpdate("CREATE TABLE IF NOT EXISTS overall_butterfly_effect (chaotic_system_id TEXT, groupIndex DOUBLE, overallSum DOUBLE, PRIMARY KEY (chaotic_system_id, groupIndex))");
        statement.close();
        
        ResultSet resultSet = statement.executeQuery("SELECT chaotic_system_id, distance, sum(evolution_count) AS evolution_sum FROM butterfly_effect GROUP BY chaotic_system_id, distance");
        while (resultSet.next()) {
            PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO overall_butterfly_effect (chaotic_system_id, groupIndex, overallSum) VALUES (?,?,?)");
    ...
    private void overallOccurenceVariation() throws SQLException{
        statement = connection.createStatement();
        statement.executeUpdate("CREATE TABLE IF NOT EXISTS overall_nbOccurence_levelVariation (chaotic_system_id TEXT, groupIndex DOUBLE, overallSum DOUBLE, PRIMARY KEY (chaotic_system_id, groupIndex))");
        statement.close();
        ...
        while (resultSet.next()) {
            PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO overall_nbOccurence_levelVariation (chaotic_system_id, groupIndex, overallSum) VALUES (?,?,?)");
            insertStatement.setString(1, resultSet.getString("chaotic_system_id"));
